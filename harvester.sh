...
while true; do
    ...
    if [ ! $((rewards+commission)) -lt $WITHDRAW_THRESHOLD ]; then
        ...
        $BINARY tx distribution withdraw-rewards $VALOPER --from $ACCOUNT --chain-id $CHAIN_ID --commission --keyring-backend test -y --gas auto --gas-adjustment 1.5 --gas-prices $GAS_PRICES$DENOM --broadcast-mode async --node $NODE_ADDR --output json | jq .
        sleep 60
    else
    ...
    $BINARY tx staking delegate $VALOPER $diff$DENOM --from $ACCOUNT --chain-id $CHAIN_ID --keyring-backend test -y --gas auto --gas-adjustment 1.5 --gas-prices $GAS_PRICES$DENOM --broadcast-mode async --node $NODE_ADDR --output json | jq .
    sleep $INTERVAL_SEC
done